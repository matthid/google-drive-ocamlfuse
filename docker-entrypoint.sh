...
su gdfuser -l -c "google-drive-ocamlfuse -version"

# check if our config exists already
if [ "$INIT" != "true" ]; then
    echo "Starting..."
    if [ ! -e "/config/.gdfuse/default/config" ]; then
        echo "no existing google-drive-ocamlfuse config found"
        exit 1
    fi
    ...
    exec su gdfuser -l -c "google-drive-ocamlfuse -verbose -debug \"${DRIVE_PATH}\"\
    ...
    touch /tmp/tmp_out.txt
		chown ${PUID}:${PGID} -R /tmp/tmp_out.txt
    tail -f /tmp/tmp_out.txt &
		sudo -u gdfuser google-drive-ocamlfuse -verbose -debug \
		 -id "${CLIENT_ID}.apps.googleusercontent.com" \
		 -secret "${CLIENT_SECRET}" || echo "google-drive-ocamlfuse failed"
